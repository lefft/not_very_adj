...
files <- list(
  ...
  expt1_raw_data_fname = "../../data/Expt1-data_anon.csv",
  ...
  age_plot_outname = "../../plots-check/expt1_age_distro.pdf",
  bysubj_plot_outname = "../../plots-check/expt1_bysubj_response_distro.pdf",
  bypred_plot_outname = "../../plots-check/expt1_bypred_response_distro.pdf")
...
message("\nreading experiment 1 data from file:\n  >> ", 
...
#   2.2: they did the experiment more than once; or        (`double_take_ids`)
#   2.3: they perform < at chance on "extreme" items        (`inaccurate_ids`)


# 2.1 non-native ids (only valid options: "American English", "Other English")
# [identifies *1* subj to exclude] 
non_native_ids <- subj_info$subj_id[!grepl("English", subj_info$lang)]



# 2.2 id of one subj who took expt twice 
# [identifies *1* subj to exclude] 
double_take_ids <- "subj32"
...
# no one is below chance on performance metric (`length(innacurate_ids) == 0`)
...
subj_info_exclusions <- rbind(
  ...
  mutate(subj_info[subj_info$subj_id %in% double_take_ids, ], reason="dupe"),
...
message("\nwriting clean/screened expt 1 analysis dataset to file:\n  >> ", 
